...
/* cannyA7_ref.sc: EECS 222 solution A7 */
...
behavior Monitor(i_img_receiver ImgIn)
{
	...
	******************************************************************************/
	...
	void main(void)
	{
	   ...
	   Tot = Tgk+Tbx+Tby+Tde+Tma+Tnm+Tah;
	   printf("Tgk = %6.2f sec = %3.1f%%\n", Tgk, Tgk/Tot*100.0);
	   printf("Tbx = %6.2f sec = %3.1f%%\n", Tbx, Tbx/Tot*100.0);
	   printf("Tby = %6.2f sec = %3.1f%%\n", Tby, Tby/Tot*100.0);
	   printf("Tde = %6.2f sec = %3.1f%%\n", Tde, Tde/Tot*100.0);
	   printf("Tma = %6.2f sec = %3.1f%%\n", Tma, Tma/Tot*100.0);
	   printf("Tnm = %6.2f sec = %3.1f%%\n", Tnm, Tnm/Tot*100.0);
	   printf("Tah = %6.2f sec = %3.1f%%\n", Tah, Tah/Tot*100.0);
	   printf("Tot = %6.2f sec = %3.1f%%\n", Tot, Tot/Tot*100.0);
...
behavior Gaussian_Kernel(out float gaussian_kernel[WINSIZE], out int kernel_center)
{
	...
	*******************************************************************************/
	...
	Tstop = clock();
	Tgk += (double)(Tstop-Tstart) / CLOCKS_PER_SEC;
	}
};
		...
		out float tempim[SIZE])
{
	...
	Tstart = clock();
	   blur_x(ROWS, COLS);
	Tstop = clock();
	Tbx += (double)(Tstop-Tstart) / CLOCKS_PER_SEC;
	}
};
		...
		out simg smoothedim)
{
	...
	Tstart = clock();
	   blur_y(ROWS, COLS);
	Tstop = clock();
	Tby += (double)(Tstop-Tstart) / CLOCKS_PER_SEC;
	}
};

behavior Gaussian_Smooth(i_img_receiver ImgIn, out simg smoothedim)
{
	/*******************************************************************************
...
behavior Derivative_X_Y(in simg smoothedim, out simg delta_x, out simg delta_y)
{
	...
	*******************************************************************************/
	...
	Tstop = clock();
	Tde += (double)(Tstop-Tstart) / CLOCKS_PER_SEC;
	}
};
...
behavior Magnitude_X_Y(in simg delta_x, in simg delta_y, out simg magnitude)
{
	...
	*******************************************************************************/
	...
	Tstop = clock();
	Tma += (double)(Tstop-Tstart) / CLOCKS_PER_SEC;
	}
};
...
behavior Non_Max_Supp(in simg gradx, in simg grady, in simg mag, out img nms)
{
	...
	*******************************************************************************/
	...
	Tstop = clock();
	Tnm += (double)(Tstop-Tstart) / CLOCKS_PER_SEC;
	   nms = result;
	}
};
...
behavior Apply_Hysteresis(in simg mag, in img nms, i_img_sender ImgOut)
{
	...
	*******************************************************************************/
	...
	Tstop = clock();
	Tah += (double)(Tstop-Tstart) / CLOCKS_PER_SEC;
...
behavior DUT(i_img_receiver ImgIn, i_img_sender ImgOut)
{
	...
	Gaussian_Smooth  gaussian_smooth(ImgIn, smoothedim);
	...
	void main(void)
	{
	   ...
	   ****************************************************************************/
	   fsm
	   {
	      ...
	      ****************************************************************************/
	      apply_hysteresis: goto gaussian_smooth;
	   }
	}
};
