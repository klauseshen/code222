...
/* cannyA9_ref.sc: EECS 222 solution A9 */
...
#define TIME_BASE	(1 MILLI_SEC)	// print time in units of micro-seconds
#define TIME_BASE_UNIT	"ms"
#define COPT_SPEEDUP	2.5		// speedup by compiler optimization
	...
	******************************************************************************/
	...
	void main(void)
	{
	   ...
	   for(i=0; i<IMG_NUM; i++)
	   {
	      ...
	      Tdiff = t2 - Tprev;
	      Tprev = t2;
	      StageDelay = ((double)Tdiff) / ((double)1 SEC);
	      FPS = 1.0 / StageDelay;
	      printf("%9s: %7.3f seconds after previous frame, %6.3f FPS.\n",
			time2str(buf2, t2/TIME_BASE), StageDelay, FPS);
...
behavior Gaussian_Kernel(out float gaussian_kernel[WINSIZE], out int kernel_center)
{
	...
	*******************************************************************************/
	...
	void main(void)
	{
	   ...
	   ****************************************************************************/
	   waitfor 0/COPT_SPEEDUP MILLI_SEC;
		...
		in int rowStart, in int rowEnd, out float tempim[SIZE])
{
	...
	void main(void)
	{
	   waitfor (440/COPT_SPEEDUP MILLI_SEC)/8;
	   blur_x(ROWS, COLS);
	}
};

behavior BlurX(i_img_receiver ImgIn, in float kernel[WINSIZE], in int center,
		...
		in int colStart, in int colEnd, out simg smoothedim)
{
	...
	void main(void)
	{
	   waitfor (625/COPT_SPEEDUP MILLI_SEC)/8;
	   blur_y(ROWS, COLS);
	}
};
...
behavior Gaussian_Smooth(out float kernel[WINSIZE], out int center)
{
	/*******************************************************************************
...
behavior Derivative_X_Y(in simg smoothedim, out simg delta_x, out simg delta_y)
{
	...
	*******************************************************************************/
	...
	void main(void)
	{
	   waitfor 260/COPT_SPEEDUP MILLI_SEC;
			...
			out simg delta_xThrough, out simg delta_yThrough)
{
	...
	*******************************************************************************/
	...
	void main(void)
	{
	   waitfor 170/COPT_SPEEDUP MILLI_SEC;
			...
			out simg magThrough)
{
	...
	*******************************************************************************/
	void non_max_supp(int nrows, int ncols, unsigned char *result)
	{
	    ...
	    short m00,gx=0,gy=0;
	//  float mag1,mag2,xperp=0,yperp=0; // floating-point arithmetic
	    int mag1,mag2,xperp=0,yperp=0;   // fixed-point arithmetic
	   ...
	   ****************************************************************************/
	   for(rowcount=1,magrowptr=mag+ncols+1,gxrowptr=gradx+ncols+1,
	      ...
	      rowcount<=nrows-2;	// bug fix 2023, RD
	      rowcount++,magrowptr+=ncols,gyrowptr+=ncols,gxrowptr+=ncols,
	      resultrowptr+=ncols){
	      for(colcount=1,magptr=magrowptr,gxptr=gxrowptr,gyptr=gyrowptr,
	         resultptr=resultrowptr;colcount<=ncols-2;	// bug fix 2023, RD
	...
	//          xperp = -(gx = *gxptr)/((float)m00); // floating-point arithmetic
	//          yperp = (gy = *gyptr)/((float)m00);
	            gx = *gxptr;                         // fixed-point arithmetic
	            gy = *gyptr;;
	            xperp = -(gx<<16)/m00;
	            yperp = (gy<<16)/m00;
	         }
	...
	void main(void)
	{
	 ...
	 //waitfor 320/COPT_SPEEDUP MILLI_SEC;
	   waitfor 290/COPT_SPEEDUP MILLI_SEC;
...
behavior Apply_Hysteresis(in simg mag, in img nms, i_img_sender ImgOut)
{
	...
	* PURPOSE: This function follow_edges is a recursive routine that traces edges along
	* all paths whose magnitude values remain above some specifyable lower
	* threshhold.
	* NAME: Mike Heath
	* DATE: 2/15/96 (adjusted 2023, RD)
	*******************************************************************************/
	...
	*******************************************************************************/
	...
	void main(void)
	{
	   waitfor 295/COPT_SPEEDUP MILLI_SEC;
...
behavior DUT(i_img_receiver ImgIn, i_img_sender ImgOut)
{
	...
	Gaussian_Smooth  gaussian_smooth(kernel, center);
	BlurX            blurX(ImgIn, kernel, center, tempim);
	...
	*******************************************************************************/
	void main(void)
	{
	   gaussian_smooth;	// compute Gaussian kernel (only once)
